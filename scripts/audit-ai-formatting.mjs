...
import { resolveProviderBookingHref, normalizeProviderCredentialText } from '../src/app/lib/providers.js';

const origin = process.env.AUDIT_ORIGIN || 'https://drsfirst.com';
const providersOnly = process.argv.includes('--providers-only');
const output = process.env.AUDIT_AI_OUTPUT || 'artifacts/site-audit/formatting-2026-09-23/live-ai.json';
const results = [];
...
try {
  ...
  for (const entry of cases.filter((entry) => !providersOnly || entry.kind === 'provider')) {
    ...
    if (normalizeProviderCredentialText(rendered) !== rendered) failures.push('Dotted credential');
    ...
    if (entry.kind === 'provider') {
      ...
      if (!card || card.subtitle !== entry.expected || card.subtitle.includes('.')) failures.push('Provider title mismatch');
